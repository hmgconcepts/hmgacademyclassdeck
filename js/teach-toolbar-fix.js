/* teach-toolbar-fix.js — V37
   Robust topbar for HMG ACADEMY CLASS DECK / client Class Deck.
*/
(function (w, d) {
  'use strict';
  ...
  function killBlockers() {
    try {
      /* HMG STANDALONE MODE: this deck uses its own auth gate (signup /
         trial / HMG ACCESS KEY / owner lifetime account in js/auth.js).
         Only bypass the gate when the deck is embedded in a portal
         (CLASSDECK.BRAND.parentPortal set and standalone !== true) —
         the original Adewale behaviour. Otherwise leave auth alone. */
      var b = (w.CLASSDECK && w.CLASSDECK.BRAND) || {};
      var portalEmbedded = b.standalone !== true && !!b.parentPortal;
      if (portalEmbedded) {
        w.HMG_AUTH_OK = true;
        w.ACD_AUTH_OK = true;
