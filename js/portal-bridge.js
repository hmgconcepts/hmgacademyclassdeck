/* portal-bridge.js — HMG ACADEMY CLASS DECK ↔ portal (V36)
...
(function (w, d) {
  ...
  function applyTheme(p) {
    ...
    try {
      ...
      var name = p.name || (w.CLASSDECK && w.CLASSDECK.BRAND && w.CLASSDECK.BRAND.productName) || 'HMG ACADEMY CLASS DECK';
  ...
  function installChip() {
    ...
    var p = readPractice() || {};
    var studio = p.name || b.studioName || 'HMG ACADEMY';
    ...
    chip.innerHTML =
      ...
      '<span style="opacity:.85">· Class Deck</span>' +
  ...
  /* HMG STANDALONE MODE ------------------------------------------------
     The HMG ACADEMY CLASS DECK ships WITHOUT a parent portal: teachers
     sign in through the deck's own auth gate (js/auth.js — signup, 3-day
     trial, HMG ACCESS KEY, owner lifetime account). In that mode this
     bridge must NOT remove the auth gate or draw the portal chip.
     Portal mode activates automatically only when BOTH are true:
       1) the deck is configured with a parentPortal path, AND
       2) a live Supabase portal session token exists in localStorage.
     Generated client decks that DO sit inside a portal keep the full
     no-second-login behaviour unchanged. */
  function portalMode() {
    try {
      var b = (w.CLASSDECK && w.CLASSDECK.BRAND) || {};
      if (b.standalone === true) return false;
      if (!b.parentPortal) return false;
      return hasSbSession();
  ...
  function boot() {
    ...
    if (!portalMode()) return;   // standalone: original auth gate stays in charge
    killAuthGate();
    installChip();
  ...
  // Load parent PRACTICE if available (same origin, portal mode only)
  try {
    if (!w.PRACTICE && portalMode()) {
